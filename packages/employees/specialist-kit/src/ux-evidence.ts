/**
 * Evidence UX READ-ONLY — proveniencia + sanitizacao (sem fullContent).
 */
import {
  isUxSensitivePath,
...
export const UX_EVIDENCE_DOMAIN = "ux_artifacts";
export const UX_EVIDENCE_PII_BLOCKED = "UX_EVIDENCE_PII_BLOCKED";
...
export type UxEvidenceBuildResult =
...
export function isUxDenyKey(key: string): boolean {
...
export function redactUxSensitiveText(text: string): string {
  return text
...
export function containsResidualUxSecrets(text: string): boolean {
...
function stripForbiddenKeys(
  ...
  if (typeof value === "string") {
    return redactUxSensitiveText(value);
  }
...
export function sanitizeUxEvidenceForResultJson(
...
export function validateSanitizedUxEvidence(
  data: Readonly<Record<string, unknown>>,
): UxEvidenceBuildResult {
  if (typeof data.workspaceId !== "string" || !data.workspaceId.trim()) {
    return {
      ok: false,
      code: "WORKSPACE_REQUIRED",
      message: "Evidence UX exige workspaceId.",
    };
  }
  if (data.domain !== UX_EVIDENCE_DOMAIN) {
    return {
      ok: false,
      code: "DOMAIN_INVALID",
      message: "Evidence UX exige domain=ux_artifacts.",
    };
  }
  if (typeof data.artifactPath !== "string") {
    return {
      ...
      message: "Evidence UX exige artifactPath.",
    };
  }
  if (
    data.artifactPath !== "repository" &&
    data.artifactPath !== "" &&
    isUxSensitivePath(String(data.artifactPath))
  ) {
    return {
      ok: false,
      code: "PATH_FORBIDDEN",
  ...
  if (scanForUnsafe(data)) {
    return {
      ok: false,
      code: UX_EVIDENCE_PII_BLOCKED,
      message: "Evidence UX contem secrets/PII ou fullContent.",
    };
  }
  return { ok: true, data };
}

export function buildUxEvidence(input: {
  ...
  if (input.toolId === "readFile") {
    const pathCheck = validateUxReadFilePath(input.artifactPath);
    if (!pathCheck.ok) {
      return { ok: false, code: pathCheck.code, message: pathCheck.message };
    }
  }
  if (input.toolId === "listDirectory") {
    const pathCheck = validateUxListDirectoryPath(input.artifactPath);
    if (!pathCheck.ok) {
      return { ok: false, code: pathCheck.code, message: pathCheck.message };
    }
  }
  ...
  if (input.toolId === "readRepository") {
    ...
    if (Array.isArray(entries)) {
      structured.entryNames = entries
        ...
        .filter((name) => !isUxSensitivePath(name));
    }
    ...
    // Nunca persistir content/body na evidence.
  ...
  const data: Record<string, unknown> = {
    ...
    analysisType: input.analysisType ?? "ux_surface",
    summary: redactUxSensitiveText(summary),
  ...
  const sanitized = sanitizeUxEvidenceForResultJson(data);
  return validateSanitizedUxEvidence(sanitized);
}
