/**
 * Evidence Marketing READ-ONLY — proveniencia + sanitizacao (sem fullContent).
 */
import {
...
export const MARKETING_EVIDENCE_DOMAIN = "marketing_artifacts";
export const MARKETING_EVIDENCE_PII_BLOCKED = "MARKETING_EVIDENCE_PII_BLOCKED";
...
export type MarketingEvidenceBuildResult =
...
export function isMarketingDenyKey(key: string): boolean {
...
export function redactMarketingSensitiveText(text: string): string {
  return text
...
export function containsResidualMarketingSecrets(text: string): boolean {
...
function stripForbiddenKeys(value: unknown): unknown {
  if (typeof value === "string") {
    return redactMarketingSensitiveText(value);
  }
...
export function sanitizeMarketingEvidenceForResultJson(
...
export function validateSanitizedMarketingEvidence(
  data: Readonly<Record<string, unknown>>,
): MarketingEvidenceBuildResult {
  if (typeof data.workspaceId !== "string" || !data.workspaceId.trim()) {
    return {
      ok: false,
      code: "WORKSPACE_REQUIRED",
      message: "Evidence Marketing exige workspaceId.",
    };
  }
  if (data.domain !== MARKETING_EVIDENCE_DOMAIN) {
    return {
      ok: false,
      code: "DOMAIN_INVALID",
      message: "Evidence Marketing exige domain=marketing_artifacts.",
    };
  }
  if (typeof data.artifactPath !== "string") {
    return {
      ...
      message: "Evidence Marketing exige artifactPath.",
    };
  }
  if (
    data.artifactPath !== "repository" &&
    data.artifactPath !== "" &&
    isMarketingSensitivePath(String(data.artifactPath))
  ) {
    return {
      ok: false,
      code: "PATH_FORBIDDEN",
  ...
  if (scanForUnsafe(data)) {
    return {
      ok: false,
      code: MARKETING_EVIDENCE_PII_BLOCKED,
      message: "Evidence Marketing contem secrets/PII ou fullContent.",
    };
  }
  return { ok: true, data };
}

export function buildMarketingEvidence(input: {
  ...
  if (input.toolId === "readFile") {
    const pathCheck = validateMarketingReadFilePath(input.artifactPath);
    if (!pathCheck.ok) {
      return { ok: false, code: pathCheck.code, message: pathCheck.message };
    }
  }
  if (input.toolId === "listDirectory") {
    const pathCheck = validateMarketingListDirectoryPath(input.artifactPath);
    if (!pathCheck.ok) {
      return { ok: false, code: pathCheck.code, message: pathCheck.message };
    }
  }
  ...
  if (input.toolId === "readRepository") {
    summary = `Repo ${repository || "n/a"} branch=${branchRef || "n/a"}`;
    structured.owner = input.rawToolData.owner ?? null;
    structured.name = input.rawToolData.name ?? null;
    structured.primaryLanguage = input.rawToolData.primaryLanguage ?? null;
  } else if (input.toolId === "listDirectory") {
    ...
    if (Array.isArray(entries)) {
      structured.entryNames = entries
        ...
        .filter((name) => !isMarketingSensitivePath(name));
    }
    ...
    structured.hasMarketingSignal =
      /\b(marketing|campaign|landing|audience|growth)\b/i.test(content);
  ...
  const data: Record<string, unknown> = {
    domain: MARKETING_EVIDENCE_DOMAIN,
    workspaceId,
    artifactPath: input.artifactPath,
    analysisType: input.analysisType ?? "marketing_surface",
    summary: redactMarketingSensitiveText(summary),
  ...
  const sanitized = sanitizeMarketingEvidenceForResultJson(data);
  return validateSanitizedMarketingEvidence(sanitized);
}
